...
public:
  ...
  Iterator find(const Key_type& k) const {
    const Pair_type temp = {k, Value_type()};
    if (map.find(temp) != map.end()) { return map.find(temp); }
    return map.end();
  }
  ...
  Value_type& operator[](const Key_type& k) {
    ...
    return map.insert({k, Value_type()})->second;
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val) {
    Iterator temp = find(val.first);
    // std::pair<Iterator, bool> = 
    if (temp != map.end()) { return {temp, true}; }
    temp = map.insert(val);
    return {temp, false};
  }
